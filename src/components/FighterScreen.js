const FighterScreen = ({isVisible, setVisible, selectedCharacter, name}) => {
    return (
        <div onClick={() => { return null }} className='fighter-screen' style={{display : isVisible ? "block" : "none", backgroundImage : `url(https://www.smashbros.com/assets_v2/img/fighter/${selectedCharacter.name.toLowerCase()}/bg.jpg)`}}>
            <a  href='#' className="close=button" onClick={() => {setVisible(false)}}/>
